...
module NodeMap = Map.Make(struct type t = node let compare = compare end)

let append_char c s = List.of_seq (String.to_seq s) @ [c]
  |> List.to_seq |> String.of_seq;;

module Trie = struct
  type trie = {
    root: node;
    map: node_value NodeMap.t;
  }
  ...
  *)
  ...
  let try_add_node node is_word trie =
    ...
    | Some node_in_map -> if is_word && not node_in_map.is_word then
  ...
  let add_word (word: string) (trie: trie) =
    let rec aux (acc: string) (curr: char option) (rest: string) (prev_node: node) (trie: trie) =
      let () = match curr with
      | Some c -> Printf.printf "%s %c %s\n" acc c rest
      | None -> Printf.printf "%s '' %s\n" acc rest in        
      
      match rest with
      | "" -> print_endline "done"; trie
      | _ -> let (curr', rest') = (String.get rest 0, String.sub rest 1 (String.length rest - 1)) in
        Printf.printf "\ncurr: %c rest: %s\n" curr' rest';
    ...
    in aux "" None word trie.root trie
end

let a = 4;;
let test = Trie.empty |> Trie.add_word "a" |> Trie.add_word "abc" |> Trie.add_word "bxy" |> Trie.add_word "b" ;;
