...
let get_neighbors (row, col) (height, width) =
  let in_bounds (row, col) = row >= 0 && row < height && col >= 0 && col < width in
  let possibles = [
    row - 1, col; row + 1, col;
    row, col - 1; row, col + 1;
  ] in
...
let get_neighbors_with_directions n dim =
  ...
  List.map (fun (row, col) -> (row, col, find_direction_of n (row, col))) neighbors
...
module Graph = struct
  ...
  let get_edges node g = 
    let is_match (a, _, _) = 
      a = node in
    List.filter is_match g.edges
    |> List.map (fun (_, b, w) -> (b, w))

  let get_all_edges_from_pos ((a, b): int*int) g = 
    List.map ( fun d -> get_edges (a, b, d) g) [N; E; S; W] 
    |> List.flatten
  ...
  let add_node_interdirectionality (intint: int*int) (directions: direction list) (g: 'a graph) : 'a graph =
    ...
    let generate_bi_edges x s = List.map (fun n -> [n, x, 1000; x, n, 1000]) s |> List.flatten in
    let rec aux nodes g =
  ...
  let from (data: bool list list) =
    ...
    let get_local_neighbors node = get_neighbors node (height, width) |> List.filter (fun n -> List.mem n nodes) in
    (* for every node, find neighbors, find directions, add interdirectionality, add directional edges *)
    let g = List.fold_left (fun acc curr -> (
      let ns = get_local_neighbors curr in
      let directions = get_directions curr ns |> get_directions_with_complements in
    ...
    let parse (input : string) : ('a graph * 'a * (int*int)) =
      let data = input |> String.split_on_char '\n' |> List.map (fun s -> List.init (String.length s) (fun i -> s.[i] = '.' || s.[i] = 'S' || s.[i] = 'E')) in
      let start = ref (0, 0, E) in
      let goal = ref (0, 0) in
      let update_start_and_goal row col c =
        if c = 'S' then start := (row, col, E)
        else if c = 'E' then goal := (row, col) in
      List.iteri (fun row line -> List.iteri (fun col c -> update_start_and_goal row col c) (String.to_seq line |> List.of_seq)) (String.split_on_char '\n' input);
      let g = from data in
      (g, !start, !goal)
  ...
  let dijkstra (start: 'a) (goal: int*int) (g: 'a graph) =
    ...
    while !pq <> PQ.empty do
      ...
      if (cx, cy) = goal then ( 
        ...
        let next_with_costs = List.map (fun (node, weight) -> (curr_cost + weight, node)) neighbors in
        let updated_queue = List.fold_left (fun acc curr -> 
          let (cost, node) = curr in
          let edge = (node, curr_node, cost) in
          trace := add_edge edge !trace;
          PQ.enqueue curr acc
          ) !pq next_with_costs in
        pq := updated_queue;
...
(* takes a list like: [((0, 1, W), 2001); ((0, 1, N), 2001); ((0, 1, E), 1001)]
   ...
   let cheapest_edges edges =
    let min = List.fold_left (fun acc (_, c) -> if c < acc then c else acc) Int.max_int edges in
